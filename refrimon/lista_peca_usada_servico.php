...
<?php
   ...
   if(!$aviso)
   {
      ...
      $acsbd = new AcsBD();
      $acsbd->conectar();
      $lista_pecas=$acsbd->obtem_lista_peca_usada_servico($data_inicio_mysql,$data_fim_mysql);
      $acsbd->desconectar();
   }
?>
...
<h2>
<?php
   echo("Pe&ccedil;as usadas em Servi&ccedil;o");
?>
</h2><br><br>
<center>
<form name='form_lista' action='ctl_lista_peca_usada_servico.php' method=post>
...
<?php
   if($aviso)
      ...
      if(empty($lista_pecas))
         echo("N&atilde;o h&aacute; pe&ccedil;as usadas em servi&ccedil;os");
      else
      {
         echo("<table border=1>");
